...
function Add($data)
{
    return db_insert('tbl_service', $data);
}
...
function getList($start = 1, $num_per_page = 5, $where = "")
{
    ...
    if (!empty($where))
        $where = "WHERE {$where}";
    $getList = db_fetch_array("SELECT * FROM `tbl_service` {$where} LIMIT {$start}, $num_per_page");
    return $getList;
}
...
function deleteCat($id){
    return db_delete('tbl_content_service',"`id` = '{$id}'");
}
//end event cat