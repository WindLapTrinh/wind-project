...
function Add($data)
{
    return db_insert('tbl_new_we', $data);
}

function Update($id, $data){
    return db_update('tbl_new_we', $data, "`id` = '{$id}'");
}

function Delete($id){
    return db_delete('tbl_new_we', "`id` = '{$id}'");
}
...
function getList($start = 1, $num_per_page = 5, $where = "")
{
    ...
    if (!empty($where))
        $where = "WHERE {$where}";
    $getList = db_fetch_array("SELECT * FROM `tbl_new_we` {$where} LIMIT {$start}, $num_per_page");
    return $getList;
}

function getListId($id){
    $item = db_fetch_row("SELECT * FROM `tbl_new_we` WHERE `id`= '{$id}'");
    if (!empty($item))
        return $item;
}
function getListSearch($search){
    $item = db_fetch_array("SELECT * FROM `tbl_new_we` WHERE `content` LIKE '%$search%' OR `name` LIKE '%$search%'");
    if (!empty($item))
        return $item;
}
